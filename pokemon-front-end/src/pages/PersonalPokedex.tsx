...
const PersonalPokedex = () => {
    const [pokemonList, setPokemonList] = useState<IPokedexData[]>([]);
    ...
    const fetchPokedexList = async () => {
        ...
        const mappedPokemonList = pokedexList.map((entry: IPokedexData) => ({
            pokemon: {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await fetchPokedexList();
                setPokemonList(data);
            } catch (error: any) {
    ...
    const renderList = () => {
        ...
        return pokemonList.length > 0 ? (
            pokemonList.map((pokemon) => (
                <Fragment key={pokemon.idPokedex}>
                    <PokedexCard pokemonData={pokemon} />
                </Fragment>
            ))
        ) : (
    ...
    return (
        <div className="mt-[64px]">
            ...
            >
                Aggiungi i Pokémon che hai già catturato
            </h2>
